...
const ItemDetail = ({descripcion}) => {
    ...
    const miSetCont = () => {
        setCont(prevCount => prevCount + cont)
    }
    const setContCero = () => {
        setCont(0)
    }
            ...
            <div className="produc-desc" id={id}>
                {descripcion.filter(desc => desc.id.toString() === id).map((desc, index) => (
                    <div key={index}className="full-card">
                        ...
                        <Contador initial={0} stock={descripcion} miDesc={desc} changeCont={cont => miSetCont} carritoActual={cont} compras={COMPRA_FINAL} funcionBoton={navigateTo} onAdd={function() {console.log("Accion realizada exitosamente")}}/>
                        ...
                        {botones === COMPRA_NO_FINAL && cont !== 0 ? miSetCont : null}
                    </div>
                ))}
            </div>
        </>
