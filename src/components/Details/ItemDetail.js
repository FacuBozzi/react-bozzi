            ...
            <div className="produc-desc" id={id}>
                {descripcion.filter(desc => desc.id.toString() === id).map((desc, index) => (
                    <div key={index}className="full-card">
                        ...
                        <Contador initial={0} stock={descripcion} miDesc={desc} changeCont={(cont) => setCont(prevCount => prevCount + cont)} carritoActual={cont} compras={COMPRA_FINAL} agregarCarrito={addItem} desc={desc} funcionBoton={navigateTo} onAdd={function() {console.log("Accion realizada exitosamente")}}/>
